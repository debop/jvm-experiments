...
class Logger(val logger: Slf4jLogger) {
	...
	@inline final def toJavaArgs(@varargs args: Any*) = args.map(_.asInstanceOf[AnyRef])
	...
	@inline final def trace(format: String, @varargs arguments: Any*) {
		if (isTraceEnabled)
			logger.trace(format, toJavaArgs(arguments): _*)
	}

	@inline final def trace(marker: Marker, format: String, @varargs arguments: Any*) {
		if (isTraceEnabled(marker))
	...
	@inline final def debug(format: String, @varargs arguments: Any*) =
		if (isDebugEnabled)
			logger.debug(format, toJavaArgs(arguments): _*)

	@inline final def debug(marker: Marker, format: String, @varargs arguments: Any*): Unit =
		if (isDebugEnabled(marker))
	...
	@inline final def info(format: String, @varargs arguments: Any*) =
		if (isInfoEnabled)
			logger.info(format, toJavaArgs(arguments): _*)

	@inline final def info(marker: Marker, format: String, @varargs arguments: Any*): Unit =
		if (isInfoEnabled(marker))
	...
	@inline final def warn(format: String, @varargs arguments: Any*) {
		if (isWarnEnabled)
			logger.warn(format, toJavaArgs(arguments): _*)
	}

	@inline final def warn(marker: Marker, format: String, @varargs arguments: Any*) {
		if (isWarnEnabled(marker))
	...
	@inline final def error(format: String, @varargs arguments: Any*) {
		if (isErrorEnabled)
			logger.error(format, toJavaArgs(arguments): _*)
	}

	@inline final def error(marker: Marker, format: String, @varargs arguments: Any*) {
		if (isErrorEnabled(marker))
