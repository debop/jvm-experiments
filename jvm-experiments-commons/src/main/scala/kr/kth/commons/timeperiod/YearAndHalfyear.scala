 ...
 * Date: 12. 12. 26
 */
class YearAndHalfyear(val year: Int, val halfyear: HalfYearKind) extends ValueObjectBase with Comparable[YearAndHalfyear] {
...
object YearAndHalfyear {
	...
	def apply(today: DateTime): YearAndHalfyear = {
		...
		new YearAndHalfyear(today.getYear, 1)
	}
}
