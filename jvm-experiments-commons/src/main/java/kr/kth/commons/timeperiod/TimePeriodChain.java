...
/**
 * kr.kth.commons.timeperiod.TimePeriodChain
...
public class TimePeriodChain extends TimePeriodContainer implements ITimePeriodChain {
	...
	public boolean add(ITimePeriod item) {
		...
		if (log.isDebugEnabled())
			log.debug("Period [{}]를 Chain의 끝에 추가합니다.", item);
	...
	public void add(int index, ITimePeriod item) {
		...
		if (log.isDebugEnabled())
			log.debug("Chain의 인덱스[{}]에 요소 [{}]fmf 삽입합니다...", index, item);
	...
	public boolean remove(Object item) {
		...
		if (log.isDebugEnabled())
			log.debug("요소 ITimePeriod [{}] 를 제거합니다...", item);
		...
		if (isRemoved && next != null) {
			if (log.isDebugEnabled())
				log.debug("요소[{}]를 제거하고, Chain의 후속 Period 들의 기간을 조정합니다...", item);
		...
		if (log.isDebugEnabled())
			log.debug("요소[{}]를 제거했습니다. 제거결과=[{}]", item, isRemoved);
	...
	public ITimePeriod remove(int index) {
		if (log.isDebugEnabled())
			log.debug("index [{}] 의 요소를 제거합니다...", index);
		Guard.shouldBeInRange(index, 0, size(), "index");
	...
	protected void assertSpaceBefore(DateTime moment, long duration) {
		if (log.isDebugEnabled())
			log.debug("moment=[{}] 이전에 duration=[{}] 만큼의 공간이 있는지 확인합니다.", moment, duration);
	...
	protected void assertSpaceAfter(DateTime moment, long duration) {
		if (log.isDebugEnabled())
			log.debug("moment=[{}] 이후에 duration=[{}] 만큼의 공간이 있는지 확인합니다.", moment, duration);
