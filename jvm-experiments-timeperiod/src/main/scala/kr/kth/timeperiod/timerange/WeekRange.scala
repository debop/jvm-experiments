...
abstract class WeekTimeRange(start: DateTime, weekCount: Int, calendar: ITimeCalendar)
    ...
    private val _startWeek: Int = WeekTools.getYearAndWeek(start, calendar).weekOfYear
    ...
    def this(period: ITimePeriod, calendar: ITimeCalendar) {
        this(period.getStart.getYear, WeekTools.getYearAndWeek(period.getStart, calendar).weekOfYear, calendar)
    }
