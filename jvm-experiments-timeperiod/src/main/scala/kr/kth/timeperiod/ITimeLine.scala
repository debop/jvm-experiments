...
import kr.kth.timeperiod.timeline.TimeLineTool
...
class TimeLine[T <: ITimePeriod](val periodClass: Class[T],
    ...
    private val _limits = if (alimits != null) TimeRange(alimits) else TimeRange(periods)
    ...
    def calculateGaps: ITimePeriodCollection = {
        ...
        getPeriods
        .filter(p => _limits.intersectsWith(p))
