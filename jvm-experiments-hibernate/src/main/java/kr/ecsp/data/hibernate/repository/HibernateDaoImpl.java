...
public class HibernateDaoImpl<E extends StatefulEntity> implements HibernateDao<E> {
	...
	protected final PagedList<E> getPage(Criteria criteria, int pageNo, int pageSize, Order... orders) {
		...
		final Criteria countCriteria = criteria;
	...
	public PagedList<E> getPage(Query query, int pageNo, int pageSize, HibernateParameter... parameters) {
		long itemCount = count(query, parameters);
	...
	public <TProject> TProject reportOne(Class<TProject> projectClass, ProjectionList projectionList, Criteria criteria) {
		Criteria projectCriteria = buildProjectionCriteria(projectClass,
		                                                   criteria,
		                                                   projectionList,
		                                                   true);
	...
	@Override
	public <TProject> List<TProject> reportList(Class<TProject> projectClass, ProjectionList projectionList, DetachedCriteria dc) {
		return reportList(projectClass, projectionList, dc.getExecutableCriteria(getSession()));
	}

	@Override
	public <TProject> List<TProject> reportList(Class<TProject> projectClass, ProjectionList projectionList, Criteria criteria) {
		Criteria projectCriteria = buildProjectionCriteria(projectClass,
		                                                   criteria,
		                                                   projectionList,
		                                                   false);
